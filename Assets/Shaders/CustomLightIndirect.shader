Shader "Custom/CustomLightIndirect"
{
    Properties
    {
    ...
    { 
        ...
        { 
            ...
            #pragma instancing_options procedural:SetupDummy
            #pragma target 4.5
            ...
            void SetupDummy()
            {
                    ...
                    // Вытаскиваем матрицу для текущего инстанса
                    float4x4 data = _InstanceBuffer[unity_InstanceID];
                    
                    // Жестко переопределяем встроенную матрицу Unity
                    unity_ObjectToWorld = data;
                    
                    // Нам также нужно инвертировать матрицу для корректного расчета нормалей (worldToObject)
                    // Быстрый способ для шейдеров (простой инверс TRS):
                    unity_WorldToObject = data;
                    unity_WorldToObject._14_24_34 *= -1.0; // Инвертируем позицию для базового освещения
                #endif
            }
            ...
            Varyings Vertex(Attributes v)
            {
                ...
                o.viewDirTS = half3(normalize(mul(tbn, viewDirWS)));
            ...
            float Dither8x8(float2 screenPos)
            {
                ...
                float2 signCorrectedPos = floor(screenPos);
        ...
        { 
            ...
            void SetupDummy()
            {
                    ...
                    unity_ObjectToWorld = data;
                    unity_WorldToObject = data;
                    unity_WorldToObject._14_24_34 *= -1.0;
                #endif
            }
            ...
            { 
                ...
                float3 worldPos = TransformObjectToWorld(v.positionOS.xyz);
                    ...
                    worldPos = ApplyShadowBias(worldPos, _LightDirection, normalWS);
                #endif
